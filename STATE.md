...
- Remote `main` is `25aed9f` with the allocation declaration, Helius ingestion, drift reconciliation, and public allocation panel deployed. The older local `main` worktree has diverged and is not the release source.
...
- Main workflow run `29658213959` passed quality, the Robinhood fork, and production deployment for `25aed9f`. Vercel deployment `dpl_9jbA6T7KyxBxrK9U5m27GdCzpEZX` is READY and aliased to lckd.tech.
- Branch `codex/release-truthfulness` stages a fail-closed public-data hotfix from current remote `main`: `/stats` uses only the vetted database RPC, unavailable counters render as unknown rather than zero, and current confirmed-only allocation classifications render as provisional signals. Historical lock records no longer claim a current balance. It passes lint, typecheck, 184 tests, production build, a direct read-only RPC check, and eight desktop/mobile Chromium route checks. It is not merged or deployed.
...
- The truthfulness hotfix preserves the existing industrial UI while replacing unavailable hero counters with `--`, relabeling provisional allocation totals as signals, and marking recent movements provisional. The independent review findings are addressed. Desktop and mobile home captures show no overflow or console errors.

## Open release gates

- Merge and deploy `codex/release-truthfulness`; then verify `/api/v1/stats` and one allocation-bearing token page on the deployed artifact.
