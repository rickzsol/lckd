...
Known follow-ups: the GitHub Actions billing lock blocks CI deploys; several files carry mixed CRLF/LF endings producing noisy diffs. Resolved 2026-07-18: the repo card renders via `loadRepoCard`, and the stale local main was fast-forwarded with the old worktree WIP preserved on `rescue/trudev-worktree-20260718`.
...
Wide-monitor pass + developer directory (2026-07-18): `/token/[id]` restructured for large displays: stats strip above a full-width chart, then a balanced card grid (lock record beside a 2/3-width developer profile, then a submitted-links card combining repo + live product beside swap; swap stretches to a horizontal full-row card when no links exist). Container grows to 1480px at 2xl. `ContributionGraph` replaced its fixed 8px cells with a fluid single CSS grid that stretches to the card (530px min-width scrolls on mobile). `TokenImage` now uses the Next image optimizer for hosts mirrored from remotePatterns (unknown hosts stay unoptimized instead of crashing); token-page dev avatars load from avatars.githubusercontent.com. New public `/developers` directory lists devs behind verified launches (`getVerifiedDevelopers` in `src/lib/profile.ts`; cards link to `/dev/[username]`), added to the navbar and sitemap; `getAccountAge` extracted to `src/lib/accountAge.ts`. Local `trudev/.env.local` was repointed at lckd-production via Supabase CLI keys (Vercel env pull returns `[SENSITIVE]` placeholders). The stale local main was fast-forwarded to origin/main and the old worktree WIP preserved on `rescue/trudev-worktree-20260718` (not merged; origin/main had already landed that work properly). Verified via Playwright at 2560/1920/1440/390 against production data.
